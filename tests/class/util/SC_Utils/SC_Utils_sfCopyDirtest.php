...
class SC_Utils_sfCopyDirTest extends Common_TestCase
{
    ...
    // ///////////////////////////////////////
    public function testSfCopyDirディレクトリでない場合Falseを返し何もしない()
    {
    ...
    public function testSfCopyDirコピー先のディレクトリが存在しない場合新たに作成する()
    {
        mkdir(self::$TMP_DIR.'/src', 0777, true);
        $fp = fopen(self::$TMP_DIR.'/src/test.txt', 'w');
        fwrite($fp, 'hello');
    ...
    public function testSfCopyDirコピー先のディレクトリが存在する場合そのままコピーする()
    {
        mkdir(self::$TMP_DIR.'/src', 0777, true);
    ...
    public function testSfCopyDir上書きフラグがONの場合同名ファイルが上書きされる()
    {
        mkdir(self::$TMP_DIR.'/src', 0777, true);
        mkdir(self::$TMP_DIR.'/dst', 0777, true); // コピー先も作成しておく
        $fp = fopen(self::$TMP_DIR.'/src/test.txt', 'w');
    ...
    public function testSfCopyDir上書きフラグがONかつ書き込み権限がない場合同名ファイルが上書きされない()
    {
        mkdir(self::$TMP_DIR.'/src', 0777, true);
        mkdir(self::$TMP_DIR.'/dst', 0777, true); // コピー先も作成しておく
        $fp = fopen(self::$TMP_DIR.'/src/test.txt', 'w');
