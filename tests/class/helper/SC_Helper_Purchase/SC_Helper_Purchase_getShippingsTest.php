...
class SC_Helper_Purchase_getShippingsTest extends SC_Helper_Purchase_TestBase
{
    ...
    // ///////////////////////////////////////
    public function testGetShippings存在しない受注IDを指定した場合結果が空になる()
    {
    ...
    public function testGetShippings存在する受注IDを指定した場合結果が取得できる()
    {
    ...
    public function testGetShippings商品取得フラグをOFFにした場合結果に商品情報が含まれない()
    {
        $order_id = $this->order_ids[0];
        $arrCustomer = $this->objQuery->getRow('*', 'dtb_customer', 'customer_id = ?', [$this->customer_ids[0]]);
        ...
        $this->actual['shipment_item_count'] = isset($result['1']) && is_array($result['1']['shipment_item']) ? count($result['1']['shipment_item']) : 0;
        $this->verify('配送情報');
    }
