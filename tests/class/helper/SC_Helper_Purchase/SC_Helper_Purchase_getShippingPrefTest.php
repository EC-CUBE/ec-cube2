...
class SC_Helper_Purchase_getShippingPrefTest extends SC_Helper_Purchase_TestBase
{
    ...
    public function testGetShippingPref保有フラグがOFFの場合全配送情報を取得する()
    {
        $this->setUpShipping($this->getMultipleShipping());

        $this->expected = ['東京都', '沖縄県', '埼玉県'];
    ...
    public function testGetShippingPref保有フラグがONの場合商品のある配送情報のみ取得する()
    {
        $this->setUpShipping($this->getMultipleShipping());

        $this->expected = ['東京都', '沖縄県'];
