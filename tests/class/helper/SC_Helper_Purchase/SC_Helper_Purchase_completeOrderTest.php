...
class SC_Helper_Purchase_completeOrderTest extends SC_Helper_Purchase_TestBase
{
    ...
    public function testCompleteOrder顧客IDが指定されている場合購入日が更新される()
    {
    ...
    public function testCompleteOrder顧客IDが指定されていない場合特にエラーなく修了できる()
    {
...
class SC_Helper_Purchase_completeOrderMock extends SC_Helper_Purchase
{
    ...
    {
        $_SESSION['testResult']['verifyChangeCart'] = ['uniqId' => $uniqId];
    }

    public static function getOrderTemp($uniqId)
    {
        $_SESSION['testResult']['getOrderTemp'] = ['uniqId' => $uniqId];
