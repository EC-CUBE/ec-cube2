...
test.describe.serial('お問い合わせページのテストをします', () => {
  ...
  test('お問い合わせページを表示します', async( { loginPage, page } ) => {
    await page.goto(url);
    ...
    test.describe('テストを実行します[GET] @attack', () => {
      ...
      test('アクティブスキャンを実行します', async() => {
        ...
        await intervalRepeater(async( { loginPage, page } ) => await zapClient.getActiveScanStatus(scanId), 5000, page);
  ...
  test('ログイン状態を確認します', async ( { loginPage, page } ) => {
  ...
  test('お問い合わせ内容を入力します', async ({ loginPage, page }) => {
   ...
   */
  test('エラーメッセージの表示を確認します', async ({ loginPage, page }) => {
    await page.goto(PlaywrightConfig?.use?.baseURL ?? '/');       // ログアウトしてしまう場合があるので一旦トップへ遷移する
    await page.goto(url);
    await page.click('input[name=confirm]');
