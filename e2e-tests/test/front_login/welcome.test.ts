...
test.describe('トップページのテストをします', () => {
  ...
  test('TOPページが正常に見られているかを確認します', async ({ loginPage, page }) => {
    await page.goto(url);
  ...
  test('body の class 名出力を確認します', async ({ loginPage, page }) => {
    await page.goto(url);
    await expect(page.locator('body')).toHaveAttribute('class', 'LC_Page_Index');
  });

  test('システムエラーが出ていないのを確認します', async ({ loginPage, page }) => {
    await page.goto(url);
    await expect(page.locator('.error')).not.toBeVisible();
  });

  test('ログアウトします', async ({ loginPage, page }) => {
    await page.goto(url);
    await loginPage.logout();
    await expect(loginPage.loginButton).toBeVisible();
  });

  test('LC_Page_Index_Ex クラスのテストをします @extends', async ( { loginPage, page }) => {
    await page.goto(url);
