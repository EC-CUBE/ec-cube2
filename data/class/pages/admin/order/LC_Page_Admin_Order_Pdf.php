...
class LC_Page_Admin_Order_Pdf extends LC_Page_Admin_Ex
{
    ...
    {
        $objFormParam->addParam('注文番号', 'order_id', INT_LEN, 'n', ['EXIST_CHECK', 'MAX_LENGTH_CHECK', 'NUM_CHECK']);
        $objFormParam->addParam('注文番号', 'pdf_order_id', INT_LEN, 'n', ['MAX_LENGTH_CHECK', 'NUM_CHECK']);
        $objFormParam->addParam('発行日', 'year', INT_LEN, 'n', ['EXIST_CHECK', 'MAX_LENGTH_CHECK', 'NUM_CHECK']);
        $objFormParam->addParam('発行日', 'month', INT_LEN, 'n', ['EXIST_CHECK', 'MAX_LENGTH_CHECK', 'NUM_CHECK']);
        $objFormParam->addParam('発行日', 'day', INT_LEN, 'n', ['EXIST_CHECK', 'MAX_LENGTH_CHECK', 'NUM_CHECK']);
        $objFormParam->addParam('帳票の種類', 'type', INT_LEN, 'n', ['EXIST_CHECK', 'MAX_LENGTH_CHECK', 'NUM_CHECK']);
        $objFormParam->addParam('ダウンロード方法', 'download', INT_LEN, 'n', ['EXIST_CHECK', 'MAX_LENGTH_CHECK', 'NUM_CHECK']);
        $objFormParam->addParam('帳票タイトル', 'title', STEXT_LEN, 'KVa', ['MAX_LENGTH_CHECK']);
        $objFormParam->addParam('帳票メッセージ1行目', 'msg1', STEXT_LEN * 3 / 5, 'KVa', ['MAX_LENGTH_CHECK']);
        $objFormParam->addParam('帳票メッセージ2行目', 'msg2', STEXT_LEN * 3 / 5, 'KVa', ['MAX_LENGTH_CHECK']);
        $objFormParam->addParam('帳票メッセージ3行目', 'msg3', STEXT_LEN * 3 / 5, 'KVa', ['MAX_LENGTH_CHECK']);
        $objFormParam->addParam('備考1行目', 'etc1', STEXT_LEN, 'KVa', ['MAX_LENGTH_CHECK']);
        $objFormParam->addParam('備考2行目', 'etc2', STEXT_LEN, 'KVa', ['MAX_LENGTH_CHECK']);
        $objFormParam->addParam('備考3行目', 'etc3', STEXT_LEN, 'KVa', ['MAX_LENGTH_CHECK']);
        $objFormParam->addParam('ポイント表記', 'disp_point', INT_LEN, 'n', ['EXIST_CHECK', 'MAX_LENGTH_CHECK']);
    }
    ...
    public function lfCheckError(&$objFormParam)
    {
        ...
        $objError->doFunc(['発行日', 'year', 'month', 'day'], ['CHECK_DATE']);
        ...
        $date_check = $objQuery->select('create_date', 'dtb_order', 'order_id = ?', [$order_max]);
