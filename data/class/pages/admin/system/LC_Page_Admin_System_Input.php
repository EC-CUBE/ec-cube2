...
class LC_Page_Admin_System_Input extends LC_Page_Admin_Ex
{
    ...
    {
        $objFormParam->addParam('メンバーID', 'member_id', INT_LEN, 'n', ['NUM_CHECK']);
        $objFormParam->addParam('名前', 'name', STEXT_LEN, 'KV', ['EXIST_CHECK', 'MAX_LENGTH_CHECK']);
        $objFormParam->addParam('所属', 'department', STEXT_LEN, 'KV', ['MAX_LENGTH_CHECK']);
        $objFormParam->addParam('ログインID', 'login_id', '', '', ['EXIST_CHECK', 'GRAPH_CHECK']);
        $objFormParam->addParam('変更前ログインID', 'old_login_id', '', '', ['GRAPH_CHECK']);
        if ($mode == 'edit' && $arrParams['password'] == DEFAULT_PASSWORD) {
            $objFormParam->addParam('パスワード', 'password', '', '', ['EXIST_CHECK']);
            $objFormParam->addParam('パスワード(確認)', 'password02', '', '', ['EXIST_CHECK']);
        } else {
            $objFormParam->addParam('パスワード', 'password', '', '', ['EXIST_CHECK', 'PASSWORD_CHAR_CHECK']);
            $objFormParam->addParam('パスワード(確認)', 'password02', '', '', ['EXIST_CHECK', 'PASSWORD_CHAR_CHECK']);
        }
        $objFormParam->addParam('権限', 'authority', INT_LEN, '', ['EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK']);
        $objFormParam->addParam('稼働/非稼働', 'work', INT_LEN, '', ['EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK']);
        $objFormParam->addParam('ページ', 'pageno', INT_LEN, 'n', ['EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK']);
    ...
    public function validateData(&$objFormParam, &$arrParams, $mode)
    {
        ...
        if ($mode == 'new') {
            ...
            $objErr->doFunc(['ログインID', 'login_id', ID_MIN_LEN, ID_MAX_LEN], ['NUM_RANGE_CHECK']);
        } elseif ($mode == 'edit') {
            $objErr->doFunc(['パスワード', 'password', PASSWORD_MIN_LEN, PASSWORD_MAX_LEN], ['SPTAB_CHECK', 'NUM_RANGE_CHECK']);
            $objErr->doFunc(['ログインID', 'login_id', ID_MIN_LEN, ID_MAX_LEN], ['SPTAB_CHECK', 'NUM_RANGE_CHECK']);
        }
    ...
    public function getMemberData($id)
    {
        ...
        return $objQuery->getRow($columns, $table, $where, [$id]);
    }
     ...
     *
     * @return bool 登録済みならtrue, 未登録ならfalse
     */
    public function memberDataExists($where, $val)
    {
        ...
        $exists = $objQuery->exists($table, $where, [$val]);
    ...
    public function updateMemberData($member_id, $arrMemberData)
    {
        ...
        // UPDATEの実行
        $objQuery->update('dtb_member', $sqlVal, $where, [$member_id]);
    }
}
