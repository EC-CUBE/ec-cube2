...
class SOAP_Type_dateTime
{
     ...
     * Constructor.
     *
     * @param string|int $date  The timestamp or ISO 8601 formatted
     *                              date and time this object is going to
     *                              represent.
     */
    public function __construct($date = -1)
    {
        if ($date == -1) {
            $this->timestamp = time();
        } elseif (is_int($date)) {
     ...
     */
    public function toSOAP($date = null)
    {
     ...
     */
    public function toString($timestamp = 0)
    {
        if (!$timestamp) {
     ...
     *
     * @param string|int $datestr  A unix timestamp or ISO 8601 date/time
     *                                 string. If empty, this object is used.
     *
     * @return bool|array  An array with the date and time components or
     *                        false on failure.
     */
    public function _split($datestr)
    {
        if (!$datestr) {
     ...
     *
     * @param string|int $datestr  @see SOAP_Type_dateTime::_split
     *
     * @return string  The ISO 8601 formatted UTC date/time string.
     */
    public function toUTC($datestr = null)
    {
     ...
     */
    public function toUnixtime($datestr = null)
    {
        $regs = $this->_split($datestr);
        if ($regs) {
     ...
     *
     * @param string|int $date1  A unix timestamp or ISO 8601 date/time
     *                               string.
     * @param string|int $date2  A unix timestamp or ISO 8601 date/time
     *                               string. If empty, this object is used.
     *
     * @return int  The difference between the first and the second date.
     */
    public function compare($date1, $date2 = null)
    {
        if (is_null($date2)) {
            $date2 = $date1;
