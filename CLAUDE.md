...
`SC_Query::delete()` や `SC_Query::select()` は **状態を保持する** ため、テスト間で影響を与える可能性があります。
...
protected function tearDown(): void
{
    ...
    // バックアップからリストア
    foreach ($this->backupData as $row) {
...
**Playwright 1.52.0で固定** - 1.53.0以降で `getByLabel()` が動作しない問題があるため
...
```typescript
// e2e-tests/pages/admin/login.page.ts
export class AdminLoginPage {
  ...
  constructor(page: Page) {
    this.page = page;
    this.loginId = page.getByRole('textbox', { name: 'ログインID' });
  }
...
Dependabotが自動的にPRを作成しますが、**1パッケージずつ検証**します：
...
git commit -m "Dependabot: Update zaproxy, tar"
```

### Playwright更新の注意

**Playwright 1.53.0以降は使用不可** - `getByLabel()` の挙動変更により既存テストが動作しません。
...
# テスト実行順序を変更して再現性確認
docker compose exec php-mysql vendor/bin/phpunit --order-by=random
```

## 参考リンク

- [EC-CUBE 2 公式サイト](https://www.ec-cube.net/)
- [EC-CUBE 2 GitHub](https://github.com/EC-CUBE/ec-cube2)
